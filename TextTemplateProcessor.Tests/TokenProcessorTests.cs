    ...
    public class TokenProcessorTests
    {
        ...
        public void ExtractTokens_LineContainsMoreThanOneValidToken_ExtractsTokens(string text1, string text2, string text3)
        {
            ...
            _nameValidater
                ...
                .Returns(true)
                .Verifiable(Times.Once);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, NameValidater_IsValidName, 1, 2);
            TokenProcessor processor = GetTokenProcessor();
        ...
        public void ExtractTokens_TokenNameIsInvalid_EscapesTokenAndLogsMessage()
        {
            ...
            _logger
                ...
                .Verifiable(Times.Once);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, Logger_Log_Message);
            TokenProcessor processor = GetTokenProcessor();
        ...
        public void LoadTokenValues_TokenHasInvalidName_LogsMessage()
        {
            ...
            _logger
                ...
                .Callback(_verifier.GetCallOrderAction(Logger_Log_Message))
                .Verifiable(Times.Once);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, Locater_CurrentSegment_Getter);
            _verifier.DefineExpectedCallOrder(Locater_CurrentSegment_Getter, Logger_Log_Message);
            TokenProcessor processor = GetTokenProcessor();
        ...
        public void ReplaceTokens_MultipleTokenErrors_LogsAllErrors()
        {
            ...
            _nameValidater
                ...
                .Callback(_verifier.GetCallOrderAction(NameValidater_IsValidName, 3))
                .Returns(true)
                .Verifiable(Times.Once);
            _nameValidater
                .Setup(nameValidater => nameValidater.IsValidName(tokenName6))
                .Callback(_verifier.GetCallOrderAction(NameValidater_IsValidName, 4))
                .Returns(true)
                .Verifiable(Times.Once);
            _logger
                .Setup(logger => logger.Log(MsgMissingTokenName, null, null))
            ...
            _logger
                ...
                .Callback(_verifier.GetCallOrderAction(Logger_Log_Message, 3))
                .Verifiable(Times.Once);
            _logger
                .Setup(logger => logger.Log(MsgTokenValueIsEmpty, segmentName, tokenName6))
                .Callback(_verifier.GetCallOrderAction(Logger_Log_Message, 4))
                .Verifiable(Times.Once);
            _logger
                .Setup(logger => logger.Log(MsgTokenMissingEndDelimiter, null, null))
                .Callback(_verifier.GetCallOrderAction(Logger_Log_Message, 5))
                .Verifiable(Times.Once);
            _verifier.DefineExpectedCallOrder(Logger_Log_Message, NameValidater_IsValidName, 1, 1);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, Logger_Log_Message, 1, 2);
            _verifier.DefineExpectedCallOrder(Logger_Log_Message, NameValidater_IsValidName, 2, 2);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, NameValidater_IsValidName, 2, 3);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, Logger_Log_Message, 3, 3);
            _verifier.DefineExpectedCallOrder(Locater_CurrentSegment_Getter, Logger_Log_Message, 0, 3);
            _verifier.DefineExpectedCallOrder(Logger_Log_Message, NameValidater_IsValidName, 3, 4);
            _verifier.DefineExpectedCallOrder(NameValidater_IsValidName, Logger_Log_Message, 4, 4);
            _verifier.DefineExpectedCallOrder(Logger_Log_Message, Logger_Log_Message, 4, 5);
            TokenProcessor processor = GetTokenProcessor();
